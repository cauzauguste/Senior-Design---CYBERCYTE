...
async def get_events(limit: int = 500):
    """Return recent raw events (anomaly or not)."""
    conn = get_db_connection()
    query = """
    ...
    return df.to_dict(orient="records")
@app.get("/mitigations")
async def get_mitigations(limit: int = 200):
    """Return all AI-reviewed anomaly events (Gemini results)."""
    conn = get_db_connection()
    query = """
        SELECT 
            e.id, e.timestamp, e.source, e.event_type, e.raw_data,
        ...
        INNER JOIN events e ON e.id = td.event_id
        ORDER BY e.timestamp DESC
        LIMIT ?
    """
    df = pd.read_sql_query(query, conn, params=(limit,))
